...
STRATEGY_SCRAPPER = 2
STRATEGY_HAWK = 3
strats = {STRATEGY_ADD_TO_CART: 'Add-To-Cart', STRATEGY_SCRAPPER: 'Scrapper', STRATEGY_HAWK: 'Hawk'}
...
def extract_info(soup, site, card_names, strategy):
    if soup is None:
        logger.info(color_msg([("[Warning] ", "d_yellow"), (str(site), "b_cyan"), (f"Soup is ", "rst"), (" None!", "d_red"), (" in extract_info (ei).", "rst")]))
        return pd.DataFrame()
    cards = []
    seen_variants = set()
    substrings_to_check = ['playmats', 'booster packs', 'booster box', 'mtg booster boxes', 'art series', 'fat packs and bundles', 'mtg booster packs', 'magic commander deck', 'world championship deck singles', 'The Crimson Moon\'s Fairy Tale', 'rpg accessories', 'scan other', 'intro packs and planeswalker decks', 'wall scrolls']
    ...
    for x in top_level:
        ...
        if content is None:
            logger.info(color_msg([("[Warning] ", "d_yellow"), (str(site), "b_cyan"), (f" class': {x}", "rst"), (" NOT found", "d_red"), (" in extract_info (ei).", "rst")]))
        else:
            break
    if content is None:
        logger.error(color_msg([("[ERROR] ", "d_red"), (str(site), "b_cyan"), (" All classes: 'content'", "rst"), (" NOT found", "d_red")]))
        if (soup.find('title')):
            logger.error(color_msg([("[Error] ", "d_red"), ("Site returned error: ", "rst"), (str(soup.find('title').text), "b_cyan")]))
            print(soup)
        return pd.DataFrame()
    products_containers = content.find_all('div', {'class': 'products-container browse'})
    if products_containers is None:
        logger.warning(color_msg([(str(site), "b_cyan"), (" Warning:", "d_yellow"), (" class': 'products-container browse", "rst"), (" NOT found", "d_red"), (" in extract_info (ei).", "rst")]))
        return pd.DataFrame()
    for products_container in products_containers:
        ...
        for item in list_items:
            ...
            if a_tag and 'yugioh' in a_tag['href']:
                value = a_tag['href']
                logger.debug(color_msg([(str(site), "b_cyan"), (" Error: ", "d_red"), ("Card isn't a magic_single: ", "rst"), (str(value), "rst")]))
                continue
            ...
            if test_title:
                ...
                card.Site = site
            logger.debug(color_msg([("[Warning] ", "b_magenta"), (" before clean:", "d_red"), (str(card.Name), "b_cyan")]))
            card.Name = clean_card_name(card.Name, card_names)
            logger.debug(color_msg([("[Warning] ", "b_magenta"), (" after clean:", "d_red"), (str(card.Name), "b_cyan")]))
            if not card.Name:
                logger.error(color_msg([("[ERROR] ", "d_red"), (str(site), "b_cyan"), ("Cant find card_name in h4: ", "rst"), (card.Name, "b_cyan")]))
            if ((card.Name not in card_names) and (card.Name.split(' // ')[0].strip() not in card_names)):
                msg = color_msg([(str(site), "b_cyan"), (" Debug:", "d_cyan"), (" Card [", "rst"), (str(card.Name), "b_yellow"), ("] NOT found", "d_red"), (" in initial list (ei).", "rst")])
                logger.debug(msg)
                continue
            if any(substring in test_category.lower() for substring in substrings_to_check):
                logger.debug(color_msg([(str(site), "b_cyan"), (" INFO: [", "d_yellow"), (str(card.Name), "rst"), ("]: is in category ", "rst"), (str(test_category), "d_cyan")]))
                continue
...
def extract_price(card, variant):
    ...
    if price_elem is not None:
        ...
        logger.error(color_msg([(str(card.Site), "b_cyan"), (f": in Fct:strategy_scrapper, Price element not found!... ", "rst"), (str(card.Name), "d_red"), (f"Problematic variant: {variant}", "rst")]))
        return 0.0

def extract_quality_language(card, variant):
    variant_description = variant.find('span', {'class': 'variant-short-info variant-description'}) or variant.find('span', {'class': 'variant-short-info'})
    if variant_description:
        quality_language = normalize_variant_description(variant_description.text)
        logger.debug(color_msg([(str(card.Site), "b_cyan"), (" [INFO]", "b_yellow"), (" Results of variant-description is: [", "rst"), (str(quality_language), "b_yellow"), ("].", "rst")]))
        return quality_language[:2]
    else:
        logger.error(color_msg([(str(card.Site), "b_cyan"), (": in extract_quality_language can\'t find variant-description ", "rst"), (str(card.Name), "d_red"), (f"Problematic variant: {variant}", "rst")]))
        return None

def extract_quantity(card, variant):
    variant_qty = variant.find('span', {'class': 'variant-short-info variant-qty'}) or variant.find('span', {'class': 'variant-short-info'})
    if variant_qty:
        variant_qty = variant_qty.text.strip()
        logger.debug(color_msg([(str(card.Site), "b_cyan"), (" [INFO]", "b_yellow"), (" Results of variant-qty is: [", "rst"), (str(variant_qty), "b_yellow"), ("].", "rst")]))
        return extract_numbers(variant_qty)
    else:
        logger.error(color_msg([(str(card.Site), "b_cyan"), (": in extract_quantity can\'t find variant-qty ", "rst"), (str(card.Name), "d_red"), (f"Problematic variant: {variant}", "rst")]))
        return None
...
def fetch_url(domain, url, retries=3, backoff_factor=1):
    for attempt in range(retries):
        ...
        if response.status_code != 200:
            ...
            logger.info(color_msg([("[INFO] ", "d_yellow"), (str(domain), "b_cyan"), (f" {response.status_code}", "b_yellow"), (" ERROR detected on GET. Retrying in ", "rst"), (f"{sleep_time}", "b_cyan"), (" seconds...", "rst")]))
...
def find_name_version_foil(place_holder):
    ...
    logger.debug(color_msg([("[INFO]", "d_yellow"), (" Card [", "rst"), (str(place_holder), "b_yellow"), ("] values are:", "rst")]))
    logger.debug(color_msg([("   product_name: [", "rst"), (str(product_name), "b_blue"), ("].", "rst")]))
    logger.debug(color_msg([("   product_version: [", "rst"), (str(product_version), "b_blue"), ("].", "rst")]))
    logger.debug(color_msg([("   product_foil: [", "rst"), (str(product_foil), "b_blue"), ("].", "rst")]))
...
def normalize_price(price_string):
    ...
    if match:
        ...
        logger.error(color_msg([("[ERROR] ", "d_red"), ("In extract_price function: problematic price: ", "rst"), (str(price_string), "d_red")]))
        return 0.0
...
def post_request(domain, url, headers, payload, retries=3, backoff_factor=1):
    for attempt in range(retries):
        ...
        if response.status_code != 200:
            sleep_time = backoff_factor * (2 ** attempt)
            logger.info(color_msg([("[INFO] ", "b_yellow"), (str(domain), "b_cyan"), (f" {response.status_code}", "b_yellow"), (" ERROR detected on POST. Retrying in ", "rst"), (f"{sleep_time}", "b_cyan"), (" seconds...", "rst")]))
            time.sleep(sleep_time)
            continue
        return response
    return None
...
def process_sites(site, card_names, strategy):
    ...
    if not soup:
        return None
    try:
        mixed_message = color_msg([("Extracting for site: ", "rst"), (str(domain), "d_cyan"), (f" ...", "rst")])
        mixed_msg_pad(mixed_message)
        temp = strats[strategy]
        mixed_message = color_msg([("Using stratey: ", "rst"), (str(temp), "d_cyan"), (".", "rst")])
        mixed_msg_pad(mixed_message)
        cards_df = extract_info(soup, domain, card_names, strategy)
        if cards_df is None or cards_df.empty:
            logger.error(color_msg([(str(domain), "b_cyan"), (" Error: ", "d_red"), (f"No cards were found using {strats[strategy]}", "rst")]))
            strategy = STRATEGY_SCRAPPER if strategy == STRATEGY_ADD_TO_CART else STRATEGY_ADD_TO_CART
            logger.info(color_msg([(str(domain), "b_cyan"), (f" Switching to {strats[strategy]} method... ", "b_red")]))
            cards_df = extract_info(soup, domain, card_names, strategy)
    except Exception as e:
        logger.exception("An error occurred")
        stack_trace = traceback.format_exc()
        logger.error(color_msg([("General exception. ", "d_red"), ("continuing ...", "rst")]))
        logger.error(color_msg([("Exception detail: \n", "d_red"), (stack_trace, "rst")]))
        return pd.DataFrame()
    return cards_df
...
def search_crystalcommerce(domain, site_url, card_names):
    ...
    try:
        ...
        if response is None:
            logger.error(color_msg([("[ERROR] ", "b_red"), (str(domain), "b_cyan"), (" Request error: ", "d_cyan"), ("Failed to fetch response after retries", "rst")]))
            return None
        if response.content:
            soup = BeautifulSoup(response.content, "html.parser")
            auth_token_elem = soup.find("input", {"name": "authenticity_token"})
            if not auth_token_elem:
                logger.info(color_msg([("[INFO] ", "b_yellow"), (str(domain), "b_cyan"), (" Authentication token not found", "rst")]))
            ...
            if response:
                ...
                logger.error(color_msg([("[ERROR] ", "b_red"), (str(domain), "b_cyan"), (" Post Request error: ", "rst"), ("Failed to post request after retries", "rst")]))
                return None
    except requests.exceptions.RequestException as e:
        logger.error(color_msg([("[ERROR] ", "b_red"), ("Request error: ", "rst"), (str(domain), "b_cyan"), (str(e), "rst")]))
        return None
...
def strategy_add_to_cart(card, variant):
    try:
        ...
        if 'data-name' not in attributes:
            return None
        unclean_name, product_version, product_foil = find_name_version_foil(attributes['data-name'])
        if not card.Foil:
        ...
        for key, value in attributes.items():
            logger.debug(color_msg([(str(key), "b_green"), (" = ", "rst"), (str(value), "b_blue")]))
        quality_language = normalize_variant_description(attributes['data-variant'])
        quality, language = quality_language[:2]
        logger.debug(color_msg([("In Fct: strategy_add_to_cart. Results are: ", "rst"), (f"{str(quality_language)}", "d_green")]))
        select_tag = variant.find('select', {'class': 'qty'}) or variant.find('input', {'class': 'qty'})
        qty_available_element = select_tag['max'] if select_tag and 'max' in select_tag.attrs else None
        if qty_available_element:
            ...
            logger.error(color_msg([("[ERROR] ", "d_red"), (f"{card.Site}: in extract_qty_available, no quantity found ", "rst"), (str(qty_available_element), "d_red"), (f" Problematic variant:  {variant}", "rst")]))
            qty_available = "0"
        ...
        logger.error(color_msg([(f"{card.Site}: in strategy_add_to_cart general Exception: {e}", "rst"), (str(product_name), "d_red"), (f"Problematic variant: {variant}", "rst")]))
        return None
...
def strategy_scrapper(card, variant):
    ...
    try:
        ...
        logger.exception("An error occurred")
        logger.error(color_msg([(str(card.Site), "b_cyan"), (f": in Fct:strategy_scrapper, {type(e).__name__}. Problematic variant: {variant}", "rst")]))
        return None
    return card
