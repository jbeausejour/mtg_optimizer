...
def upgrade():
    ...
    op.create_table('marketplace_card',
        ...
        sa.Column('set_id', sa.String(36), sa.ForeignKey('sets.id', name='fk_card_set_id')),
    ...
    op.create_table('scan',
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('created_at', sa.DateTime(), nullable=False)  # Ensure this is not nullable
    )

    # Create scan_result table
    op.create_table('scan_result',
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('scan_id', sa.Integer(), sa.ForeignKey('scan.id', name='fk_ScanResult_scan_id'), nullable=False),
        sa.Column('marketplace_card_id', sa.Integer(), sa.ForeignKey('marketplace_card.id', name='fk_ScanResult_marketplace_card_id'), nullable=False),
        sa.Column('site_id', sa.Integer(), sa.ForeignKey('site.id', name='fk_ScanResult_site_id'), nullable=False),
        sa.Column('price', sa.Float(), nullable=False)
    )
