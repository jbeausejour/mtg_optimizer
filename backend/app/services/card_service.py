...
from app.models.card import MarketplaceCard, ScryfallCardData, UserBuylistCard
...
class CardDataManager:
    ...
    def scryfall_request(url, params=None):
        ...
        try:
            ...
            logger.error("Error during Scryfall request: %s", str(e))
            return None
    ...
    def get_scryfall_suggestions(query, limit=10):
        ...
        try:
            response = requests.get(
                scryfall_api_url, params=params, timeout=REQUEST_TIMEOUT
            )
            ...
            logger.error(
                "Error fetching suggestions from Scryfall: %s", str(e))
            return []
        finally:
    ...
    def get_all_sets_from_scryfall():
        ...
        try:
            ...
            oldest_allowed_update = datetime.now(timezone.utc) - timedelta(
                days=7
            )  # Update weekly
            outdated_sets = Sets.query.filter(
                Sets.last_updated < oldest_allowed_update
            ).first()
            ...
            logger.info("Retrieved %d sets from the database", len(sets))
            return sets

        except Exception as e:
            logger.error("Error fetching sets: %s", str(e))
            return []
    ...
    def fetch_sets_from_scryfall():
        ...
        try:
            response = requests.get(
                'f"{SCRYFALL_API_BASE}/sets', timeout=REQUEST_TIMEOUT
            )
            response.raise_for_status()
            sets_data = response.json()["data"]
        except requests.RequestException as e:
            logger.error("Error fetching sets from Scryfall: %s", str(e))
            return []
        ...
        for set_info in sets_data:
            ...
            if not magic_set:
                ...
                added_count += 1
                logger.info("Adding new set: %s", set_info["code"])
            else:
                updated_count += 1
                logger.info("Updating existing set: %s", set_info["code"])
            ...
            magic_set.released_at = (
                datetime.strptime(set_info["released_at"], "%Y-%m-%d").date()
                if set_info["released_at"]
                else None
            )
        ...
        try:
            db.session.commit()
            logger.info(
                "Sets data updated successfully. Updated: %d, Added: %d",
                updated_count,
                added_count,
            )
        except Exception as e:
            db.session.rollback()
            logger.error("Error committing sets data to database: %s", str(e))

        return sets_data

    @staticmethod
    def fetch_additional_data(card_name, set_code):
        cardconduit_data = CardDataManager.fetch_cardconduit_data(
            card_name, set_code)
        purchase_data = CardDataManager.fetch_purchase_data(
            cardconduit_data.get("purchase_uris", {})
        )

        # Update the stored data
        card_data = ScryfallCardData.query.filter_by(
        ...
        try:
            ...
            wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "div.border-b.border-gray-200.py-6")
                )
            )
            ...
            wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "table.whitespace-nowrap.text-sm.border")
                )
            )

            results = []
            card_entries = driver.find_elements(
                By.CSS_SELECTOR, "div.border-b.border-gray-200.py-6"
            )

            for entry in card_entries:
                name_div = entry.find_element(
                    By.CSS_SELECTOR,
                    "div.text-sm.sm\\:text-lg.flex.items-center.flex-no-wrap",
                )
                if name_div and cardname.lower() in name_div.text.strip().lower():
                    set_info_div = entry.find_element(
                        By.CSS_SELECTOR, "div.text-xs.sm\\:text-base.font-sans"
                    )
                    set_info = set_info_div.text.strip() if set_info_div else "N/A"

                    price_div = entry.find_element(
                        By.CSS_SELECTOR, "div.p-6.xl\\:p-8.pb-0.font-mono.w-full"
                    )
                    retail_price = "Unavailable"
                    if price_div:
                        price_text = price_div.find_element(
                            By.CSS_SELECTOR, "div.text-xs.sm\\:text-base"
                        )
                        retail_price = (
                            price_text.text.strip() if price_text else "Unavailable"
                        )

                    buylist_table = entry.find_element(
                        By.CSS_SELECTOR, "table.whitespace-nowrap.text-sm.border"
                    )
                    buylist_prices = {}
                    if buylist_table:
                        rows = buylist_table.find_elements(By.TAG_NAME, "tr")[
                            1:
                        ]  # Skip header row
                        for row in rows:
    ...
    def fetch_purchase_data(purchase_uris):
        ...
        for site, url in purchase_uris.items():
            if (
                site != "cardmarket" and site != "cardhoarder"
            ):  # Temporarily disable CardMarket & cardhoarder
                purchase_data[site] = CardDataManager.fetch_marketplace_price(
                    url)
        return purchase_data

    @staticmethod
    def fetch_marketplace_price(
        url, max_retries=3, use_headless=True
    ):  # Reduced retries and added headless option
        ...
        try:
            ...
            if "tcgplayer.com" in url:
                ...
                product_info = (
                    ...
                    .until(
                        EC.presence_of_element_located(
                            (By.CSS_SELECTOR, "h1.product-details__name")
                        )
                    )
                    .text
                )
                ...
                price_table = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (
                            By.CSS_SELECTOR,
                            "section.price-points.price-guide__points table",
                        )
                    )
                )
                ...
                sales_section = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (By.CSS_SELECTOR, "section.price-guide__latest-sales")
                    )
                )
                sales = sales_section.find_elements(
                    By.CSS_SELECTOR, "div.latest-sales > div"
                )
                for sale in sales[:3]:  # Get the last 3 sales
                    date = sale.find_element(By.CSS_SELECTOR, "span.date").text
                    condition = sale.find_element(
                        By.CSS_SELECTOR, "span.condition"
                    ).text
                    quantity = sale.find_element(
                        By.CSS_SELECTOR, "span.quantity").text
                    price = sale.find_element(
            ...
            if max_retries > 0:
                ...
                return CardDataManager.fetch_marketplace_price(
                    url, max_retries - 1, use_headless
                )
            return None
    ...
    def store_card_data(data):
        card_data = ScryfallCardData(
            ...
            multiverse_ids=",".join(
                map(str, data["scryfall"].get("multiverse_ids", []))
            ),
            ...
            purchase_uris=json.dumps(
                data["scryfall"].get("purchase_uris", {})),
    ...
    def get_recent_card_data(card_name):
        ...
        recent_data = (
            ScryfallCardData.query.filter_by(card_name=card_name)
            .filter(ScryfallCardData.scan_timestamp > one_day_ago)
            .first()
        )
        ...
        if recent_data:
            return {
                "scryfall": {
                    ...
                    "multiverse_ids": list(
                        map(int, recent_data.multiverse_ids.split(","))
                    ),
    ...
    def optimize_card_purchases(strategy="milp", min_store=None, find_min_store=False):
        card_details_df = pd.read_sql(
            MarketplaceCard.query.statement, MarketplaceCard.query.session.bind
        )
        buylist_df = pd.read_sql(
            UserBuylistCard.query.statement, UserBuylistCard.query.session.bind
        )

        optimization_engine = PurchaseOptimizer(card_details_df, buylist_df)

        if strategy == "milp":
            return optimization_engine.run_milp_optimization()
