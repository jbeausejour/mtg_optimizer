...
from app.constants import CardQuality, QUALITY_WEIGHTS, LANGUAGE_WEIGHTS
...
class PurchaseOptimizer:
    ...
    def _find_best_solution(self, solutions):
        ...
        complete_solutions = [
            ...
            if sol['nbr_card_in_solution'] == len(self.user_wishlist_df)
        ]
        
        if complete_solutions:
            # Among complete solutions, choose the one with lowest cost
            return min(complete_solutions, key=lambda x: (
        ...
        return max(solutions, key=lambda x: (
    ...
    @staticmethod
    def _compute_pulp_optimization(filtered_listings_df, user_wishlist_df, unique_cards, unique_stores, costs, total_qty, min_store, find_min_store):
            ...
            if find_min_store:
                ...
                for store_count in range(1, len(unique_stores) + 1):
                    ...
                    prob, buy_vars = PurchaseOptimizer._setup_prob(
                        costs, unique_cards, unique_stores, user_wishlist_df,
                        store_count
                    )
                    
                    if pulp.LpStatus[prob.status] == "Optimal":
                        iteration_results = PurchaseOptimizer._process_result(
                            buy_vars, costs, filtered_listings_df
                        )
                        ...
                        if cards_found == total_qty:  # Complete solution
                            ...
                            if (best_complete_solution is None or 
                                actual_cost < best_complete_solution["total_price"]):
                            ...
                            if len(complete_solutions) > 1:
                                cost_improvement = (
                                    complete_solutions[-2]["total_price"] - 
                                    complete_solutions[-1]["total_price"]
                                )
                ...
                if complete_solutions:
                    ...
                    min_store_solutions = [
                        ...
                        if sol["total_price"] <= best_price * 1.05
                    ]
    ...
    def _run_pulp(filtered_listings_df, user_wishlist_df, min_store, find_min_store):
        ...
        try:
            # Setup phase
            setup_results = PurchaseOptimizer._setup_pulp_optimization(
            ...
            if len(unique_stores) < min_store:
                logger.warning(
                    f"Adjusting min_store from {min_store} to {len(unique_stores)} "
                    f"due to available stores"
                )
    ...
    def _setup_prob(costs, unique_cards, unique_stores, user_wishlist, min_store):
        ...
        prob += (
            cost_weight * pulp.lpSum(
                buy_vars[card][store] * costs[card][store] 
                for card in unique_cards 
                for store in unique_stores
            ) / total_possible_cost +
            store_weight * pulp.lpSum(store_vars[store] for store in unique_stores) / len(unique_stores)
        )
                    ...
                    if weighted_price != 10000:  # Only include cards in solution
                        ...
                        if matching_cards.empty:
                            ...
                            if not potential_matches.empty:
                                ...
                                closest_match_idx = (potential_matches["weighted_price"] - weighted_price).abs().idxmin()
    ...
    def _run_nsga_ii(self, filtered_listings_df, user_wishlist_df, config, milp_solution=None):
        ...
        for gen in range(NGEN):
            ...
            if complete_solutions:
                elite = sorted(complete_solutions, 
                            key=lambda x: x.fitness.values[0])[:ELITE_SIZE]
            ...
            while len(offspring) < POP_SIZE - len(elite):
                ...
                if complete_solutions and random.random() < 0.7:
                    ...
                    parent2 = tools.selTournament([ind for ind in pop if ind != parent1],
                                                k=1, tournsize=TOURNAMENT_SIZE)[0]
                else:
                    parent1, parent2 = tools.selTournament(pop, k=2, tournsize=TOURNAMENT_SIZE)
                
                # Crossover
                if random.random() < CXPB:
                    child1, child2 = toolbox.mate(toolbox.clone(parent1), 
                                                toolbox.clone(parent2))
                    # Mutation
            ...
            for ind in offspring:
                ...
                # Track best solution
                if (ind.fitness.values[1] >= 0.999 and  # Complete solution
                    ind.fitness.values[0] < best_fitness):  # Better cost
            ...
            current_best_cost = min((ind.fitness.values[0] 
                                for ind in pop 
                                if ind.fitness.values[1] >= 0.999), 
                                default=float('inf'))
        ...
        if best_solution is not None:
            ...
            if standardized_solution:
                ...
                for ind in sorted(archive, key=lambda x: x.fitness.values[0]):
                    iteration_solution = self._standardize_nsga_solution(
                        ind,
                        filtered_listings_df,
                        user_wishlist_df
                    )
            ...
            # Create slight variations of MILP solution
                ...
                if random.random() < 0.5:  # 50% chance to mutate
                    variant = PurchaseOptimizer._custom_mutation(
                        ...
                        filtered_listings_df,
                        user_wishlist_df,
                        indpb=0.1  # Low mutation rate for variants
                    )[0]
