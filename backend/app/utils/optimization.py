...
class PurchaseOptimizer:
    ...
    def run_optimization(card_names, config):
        ...
        card_details_df = pd.read_sql(MarketplaceCard.query.statement, db.session.bind)
    ...
    def _run_pulp(standardized_cards_df, available_cards_to_buy_df, min_store, find_min_store):
        ...
        if not find_min_store:
            prob, buy_vars = PurchaseOptimizer._setup_prob(costs, unique_cards, unique_stores, available_cards_to_buy_df, min_store)
            if pulp.LpStatus[prob.status] != 'Optimal':
                logger.warning("Solver did not find an optimal solution.")
                return None
            all_iterations_results = PurchaseOptimizer._process_result(buy_vars, costs, False, total_qty, standardized_cards_df)
            ...
            while current_min >= 1:
                ...
                prob, buy_vars = PurchaseOptimizer._setup_prob(costs, unique_cards, unique_stores, available_cards_to_buy_df, current_min)
                
                if pulp.LpStatus[prob.status] != 'Optimal':
                    logger.warning("Solver did not find an optimal solution.")
                    break
                
                iteration_results = PurchaseOptimizer._process_result(buy_vars, costs, True, total_qty, standardized_cards_df)
    ...
    def _run_nsga_ii(card_details_df, buylist, milp_solution=None):
        toolbox = PurchaseOptimizer._initialize_toolbox(card_details_df, buylist)
        ...
        if milp_solution:
            ...
            pop = PurchaseOptimizer._initialize_population_with_milp(MU, card_details_df, buylist, milp_individual)
        else:
    ...
    def _initialize_toolbox(card_details_df, buylist):
        ...
        toolbox.register("evaluate", PurchaseOptimizer._evaluate_solution_wrapper(card_details_df, buylist))
        toolbox.register("mate", PurchaseOptimizer._custom_crossover)
        toolbox.register("mutate", partial(PurchaseOptimizer._custom_mutation, card_details_df=card_details_df, buylist=buylist))
    ...
    def _initialize_population_with_milp(n, card_details_df, buylist, milp_solution):
        population = [PurchaseOptimizer._initialize_individual(card_details_df, buylist) for _ in range(n-1)]
        milp_individual = PurchaseOptimizer._milp_solution_to_individual(milp_solution)
