...
class PurchaseOptimizer:
    ...
    def run_optimization(self, card_names, sites, config):
        ...
        card_details_df = pd.read_sql(
            MarketplaceCard.query.statement, db.session.bind)
        ...
        if config["milp_strat"]:
            results, iterations = run_milp_optimization()
        elif config["nsga_algo_strat"]:
            results = run_nsga_ii_optimization()
        elif config["hybrid_strat"]:
            milp_solution, _ = run_milp_optimization()
            results = run_nsga_ii_optimization(milp_solution)
        else:
            raise ValueError(
                "No valid optimization strategy specified in config")
        ...
        if isinstance(results, pd.DataFrame):
            ...
            sites_results = [self.get_purchasing_plan(
                solution) for solution in results]
