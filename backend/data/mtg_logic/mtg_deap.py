...
def evaluate_solution_wrapper(card_details_df, buylist):
    def evaluate_solution(individual):
        ...
        stores = set()
        card_counters = {row["Name"]: row["Quantity"]
...
def initialize_population_with_milp(n, card_details_df, buylist, milp_solution):
    population = [initialize_individual(
        card_details_df, buylist) for _ in range(n - 1)]
...
def initialize_toolbox(card_details_df, buylist):
    ...
    custom_mutation_with_args = partial(
        custom_mutation, card_details_df=card_details_df, buylist=buylist
    )
...
def run_nsga_ii(card_details_df, buylist, milp_solution=None):
    ...
    if milp_solution:
        milp_individual = milp_solution_to_individual(milp_solution)
        pop = initialize_population_with_milp(
        ...
        logger.warning(
            color_msg(
                [
                    ...
                    (f"{gen}", "d_cyan"),
                    (f"]: Avg Fitness = ", "rst"),
                    (f"{avg_fitness:.2f}", "d_green"),
                    (f", Best Fitness = ", "rst"),
                    (f"{best_fitness}", "d_green"),
                ]
            )
        )
        ...
        if generations_without_improvement >= num_generations_threshold:
            logger.warning(
                color_msg(
                    [
                        ("[INFOS] ", "d_yellow"),
                        (f"Convergence reached after {gen} generations.", "rst"),
                    ]
                )
            )
            break
    ...
    for idx in solution:
        ...
        if card_name in buylist["Name"].values:
            ...
            if required_quantity > 0:
                ...
                buylist.at[
                    buylist[buylist["Name"] == card_name].index[0], "Quantity"
