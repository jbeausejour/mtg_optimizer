...
def calculate_stats(group):
    ...
    return pd.Series(
        [mean_price, std_price, idx_min], index=["mean_price", "std_price", "idx_min"]
    )
...
def check_unmapped_qualities(master_df):
    ...
    unmapped_qualities_before_mapping = [
        q for q in unique_qualities if q not in quality_mapping
    ]
    ...
    if unmapped_qualities_before_mapping:
        current_app.logger.warning(
            color_msg(
                [
                    (
                        "The following qualities were not found in the mapping dictionary: ",
                        "rst",
                    ),
        ...
        for unmapped_quality in unmapped_qualities_before_mapping:
            ...
            new_mapping = input(
                f"Enter the correct mapping for quality '{unmapped_quality}': "
            )
            ...
            if new_mapping:  # Check if user provided a non-empty string
                ...
                master_df.loc[master_df["Quality"] == unmapped_quality, "Quality"] = (
                    new_mapping
                )
...
def display_purchasing_plan(purchasing_plan):
    ...
    grouped_plan = (
        purchasing_plan.groupby(["Site", "Name"])
        .agg(
            {
                "Quantity": "sum",
                "Price": "sum",
                "Quality": lambda x: ", ".join(
                    x.unique()
                ),  # Join unique quality values
            }
        )
        .reset_index()
    )
...
def fetch_all_card_details(card_names, special_site_flag=True, save_all_research=True):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        future_to_site = {
            executor.submit(process_sites, site, card_names, strategy): (site, strategy)
            for site, strategy in sites
        }
    ...
    for future in as_completed(future_to_site):
        ...
        try:
            ...
            if cards_df is not None and not cards_df.empty:
                ...
                mixed_message = color_msg(
                    [
                        ("Extraction complete: ", "rst"),
                        (str(len(cards_df)), "d_green"),
                        (" cards found !", "rst"),
                    ]
                )
                mixed_msg_pad(mixed_message)
                ...
                current_app.logger.error(
                    color_msg(
                        [
                            (str(domain), "b_cyan"),
                            (" Error: all methods failed... ", "b_red"),
                        ]
                    )
                )
            current_app.logger.info(color_msg([(str(dash_line_message), "b_magenta")]))
...
def find_best_sites(master_df, unfulfilled_items_df):
    ...
    unique_cards = unfulfilled_items_df[
        unfulfilled_items_df["Name"].isin(
    ...
    for card_name, card_group in unique_cards.groupby("Name"):
        ...
        card_fulfillment_df = master_df[
            (master_df["Site"] == site) & (master_df["Name"] == card_name)
        ]
        total_cost += card_fulfillment_df["Price"].sum()
        best_sites.append((site, card_fulfillment_df))

    # Update unfulfilled_items_df
    unfulfilled_items_df = unfulfilled_items_df[
        ~unfulfilled_items_df["Name"].isin(unique_cards["Name"])
    ]
    ...
    while not unfulfilled_items_df.empty:
        ...
        for site_name, site_inventory_df in master_df.groupby("Site"):
            site_fulfillment_df = site_inventory_df[
                site_inventory_df["Name"].isin(unfulfilled_items_df["Name"])
            ]
        ...
        site_cards_df = master_df[
            (master_df["Site"] == best_site)
            & (master_df["Name"].isin(unfulfilled_items_df["Name"]))
        ]
        ...
        cost_optimized_cards = site_cards_df[
            site_cards_df["Price"] <= min_price * threshold
        ]
...
def prioritize_better_quality(grouped):
    ...
    for _, group in grouped:
        ...
        for i in range(1, len(group)):
            ...
            if (
                card["Weighted_Price"]
                < (best_quality_card["Weighted_Price"] * price_pct_threshold)
            ) and (card["Quantity"] > 0):
...
def read_site_list(special_site_flag):
    ...
    with open(r"./data/site_list.txt", "r") as f:
        ...
        for line in f.readlines():
            ...
            strategy = (
                site_info[1].strip() if len(site_info) > 1 else 1
            )  # Default strategy to '1' if not specified
    ...
    regular_sites = [
        (site, strategy)
        for site, strategy in sites
        if (not site.startswith("--") and not site.startswith("xx"))
    ]
    special_sites = [
        (site[2:], strategy) for site, strategy in sites if site.startswith("--")
    ]
    not_working_sites = [
        (site, strategy) for site, strategy in sites if site.startswith("xx")
    ]
...
def save_dataframe_with_retry(container, filename):
    ...
    while retry_count < max_retries:
        ...
        if retry_count < max_retries:
            choice = (
                input(
                    "Do you want to (C)ancel, choose a (D)ifferent name, or (R)etry? "
                )
                .strip()
                .lower()
            )
...
def save_the_data(unique_cards, results_path):
    ...
    unique_cards_per_site = (
        unique_cards.groupby("Site").size().reset_index(name="Count")
    )

    # Sort the sites by the number of unique cards in descending order
    unique_cards_per_site = unique_cards_per_site.sort_values(
...
def standardize_dataframe(all_cards_df):
    ...
    card_details_df["Foil"] = card_details_df["Foil"].apply(lambda x: 1 if x else 0)
